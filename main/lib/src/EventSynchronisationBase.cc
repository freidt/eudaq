...
namespace eudaq{
  ...
  void SyncBase::addBOREEvent(int fileIndex, const eudaq::DetectorEvent& BOREvent )
  {
    ...
    if (m_TLUs_found==0) {
      std::cerr << "No TLU found, faking a TLU" << std::endl;
      m_fake_TLU = true;
      m_run_number = BOREvent.GetRunNumber();
      m_EventsProFileReader.back()=BOREvent.NumEvents()+1;
      ++m_registertProducer;
      ++m_TLUs_found;
    }
  }
...
// 		}
  ...
  int SyncBase::AddDetectorElementToProducerQueue( int fileIndex,std::shared_ptr<eudaq::DetectorEvent> detEvent )
  {
    if (detEvent)
    {
      ...
      if (m_fake_TLU) {
        getFirstTLUQueue().push(std::make_shared<TLUEvent>(TLUEvent(m_run_number, m_event_counter++, 0ULL)));
      }
    }
    return true;
  }
