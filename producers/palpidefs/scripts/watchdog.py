...
def main(stdscr):
    ...
    while True:
      ...
      all_ok = True    # Found an error?
      no_grace = False # There is no grace in case of an out-of-sync
      for pad in PADS:
        print_pad(stdscr,pad)
        if (pad['bit'] == 0):
          all_ok = False
          if (pad['name'] == 'Out of Sync'):
            no_grace = True
      ...
      if (all_ok):
        ...
        if alert_masked == False:
          ...
          if alert_sent < 0:
            if not no_grace: # grace time allowed
            ...
            else: # send now, there is no grace for such errors
              time_to_alert = -1
